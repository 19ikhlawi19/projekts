...
class OnlineHandwritingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            encoded_label = online_utils.encode_label(label_str, crnn_configuration.CHAR_TO_IDX)
    ...
    def _get_fallback_item(self):
        ...
        fallback_image = torch.zeros((1, crnn_configuration.CNN_INPUT_HEIGHT, crnn_configuration.CNN_INPUT_WIDTH), dtype=torch.float32)
...
def online_collate_fn(batch):
    ...
    for item in batch:
        if isinstance(item, tuple) and len(item) == 3:
             ...
             if isinstance(img_tensor, torch.Tensor) and img_tensor.ndim == 3 and \
                img_tensor.shape == (crnn_configuration.CNN_INPUT_CHANNELS, crnn_configuration.CNN_INPUT_HEIGHT, crnn_configuration.CNN_INPUT_WIDTH) and \
                isinstance(label_indices, list) and len(label_indices) > 0 and \
                isinstance(rel_path, str) and "FALLBACK" not in rel_path:
                  valid_batch.append(item)
    if not valid_batch:
         return (torch.empty((0, crnn_configuration.CNN_INPUT_CHANNELS, crnn_configuration.CNN_INPUT_HEIGHT, crnn_configuration.CNN_INPUT_WIDTH), dtype=torch.float),
    ...
    try: batch_images = torch.stack(images, dim=0)
        ...
        logger.error(f"Stacking Fehler: {stack_e}, Shapes: {[img.shape for img in images]}"); return (torch.empty((0, crnn_configuration.CNN_INPUT_CHANNELS, crnn_configuration.CNN_INPUT_HEIGHT, crnn_configuration.CNN_INPUT_WIDTH), dtype=torch.float), torch.empty((0,), dtype=torch.long), torch.empty((0,), dtype=torch.long), [])
    ...
    except Exception as label_e: logger.error(f"Label Tensor Fehler: {label_e}, Labels: {labels}"); return (torch.empty((0, crnn_configuration.CNN_INPUT_CHANNELS, crnn_configuration.CNN_INPUT_HEIGHT, crnn_configuration.CNN_INPUT_WIDTH), dtype=torch.float), torch.empty((0,), dtype=torch.long), torch.empty((0,), dtype=torch.long), [])
...
def load_online_data_manifests():
    ...
    df_train = online_utils.load_manifest(crnn_configuration.CURRENT_TRAIN_MANIFEST)
    df_val = online_utils.load_manifest(crnn_configuration.CURRENT_VAL_MANIFEST)
...
def validate_epoch(model, data_loader, device, criterion, current_epoch_num, split_name="val"):
    ...
    device_type_str = crnn_configuration.DEVICE.split(':')[0] # 'cuda' oder 'cpu'
    # Prüfe ob AMP für Eval genutzt werden soll (kann performance bringen, aber nicht essenziell)
    amp_is_enabled_for_eval = crnn_configuration.USE_MIXED_PRECISION and device_type_str == "cuda"
    ...
    with torch.no_grad():
        ...
        for batch_images, batch_labels_flat, batch_label_lengths, batch_rel_paths in progress_bar:
            ...
            try:
                ...
                pred_strs = online_utils.decode_ctc_output(logits, crnn_configuration.IDX_TO_CHAR)
            ...
            try:
                for length in batch_label_lengths.tolist():
                    ...
                    true_str = online_utils.decode_label_list(label_indices, crnn_configuration.IDX_TO_CHAR); all_true_strings.append(true_str); current_idx = end_idx
...
def save_training_history(history_list, filename="training_history"):
    ...
    history_csv_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{filename}.csv")
    history_json_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{filename}.json")
    online_utils.create_directory(crnn_configuration.RESULTS_PATH)
...
def plot_training_curves(history_list):
    ...
    try:
        ...
        plt.tight_layout(pad=2.0); plot_save_path = os.path.join(crnn_configuration.RESULTS_PATH, "training_curves.png")
...
def train_online_model():
    ...
    try:
        device = torch.device(crnn_configuration.DEVICE); logger.info(f"Verwende Gerät: {device}")
        device_type_str = device.type # 'cuda' oder 'cpu'
        model = crnn_handwriting_model.build_online_crnn_model();
        ...
        criterion = nn.CTCLoss(blank=crnn_configuration.BLANK_IDX, reduction='mean', zero_infinity=True); logger.info(f"CTC-Loss initialisiert (Blank Index: {crnn_configuration.BLANK_IDX})")
        lr, wd = crnn_configuration.LEARNING_RATE, crnn_configuration.WEIGHT_DECAY; opt_name = crnn_configuration.OPTIMIZER.lower()
        ...
        scheduler = None; sched_name = crnn_configuration.SCHEDULER.lower(); scheduler_metric = crnn_configuration.SCHEDULER_METRIC
        scheduler_mode = 'min' if 'loss' in scheduler_metric or 'cer' in scheduler_metric or 'wer' in scheduler_metric else 'max'
        if sched_name == 'reducelronplateau': scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=scheduler_mode, factor=0.2, patience=crnn_configuration.SCHEDULER_PATIENCE, verbose=True, min_lr=1e-7)
        elif sched_name == 'cosineannealinglr': scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=crnn_configuration.EPOCHS, eta_min=1e-7)
        ...
        scaler = GradScaler(enabled=(crnn_configuration.USE_MIXED_PRECISION and device_type_str == "cuda")); logger.info(f"Mixed Precision Training (AMP) aktiviert: {scaler.is_enabled()}")
        ...
        base_path = crnn_configuration.BASE_PATH
        ...
        train_loader = DataLoader(train_dataset, batch_size=crnn_configuration.BATCH_SIZE, shuffle=True, collate_fn=online_collate_fn, num_workers=crnn_configuration.NUM_WORKERS, pin_memory=True, drop_last=True)
        val_loader = DataLoader(val_dataset, batch_size=crnn_configuration.BATCH_SIZE * 2, shuffle=False, collate_fn=online_collate_fn, num_workers=crnn_configuration.NUM_WORKERS, pin_memory=True) if val_dataset else None
        ...
        logger.info("="*50); logger.info(f"=== STARTE TRAININGSLOOP (Max {crnn_configuration.EPOCHS} Epochen) ==="); logger.info("="*50)
        es_metric_key = crnn_configuration.EARLY_STOPPING_METRIC; is_loss_metric = 'loss' in es_metric_key or 'cer' in es_metric_key or 'wer' in es_metric_key
        ...
        logger.info(f"Early Stopping: Metric='{es_metric_key}', Patience={crnn_configuration.EARLY_STOPPING_PATIENCE}, Mode='{metric_mode}'")
        for epoch in range(1, crnn_configuration.EPOCHS + 1):
            epoch_start_time = time.time(); logger.info(f"--- Epoche {epoch}/{crnn_configuration.EPOCHS} ---")
            ...
            if val_loader and crnn_configuration.EARLY_STOPPING_PATIENCE > 0:
                 ...
                 if current_metric_value is None or not np.isfinite(current_metric_value): logger.warning(f"ES Metrik '{es_metric_key}' ungültig Ep {epoch}.")
                      ...
                      if improved:
                          ...
                          best_model_save_path = os.path.join(crnn_configuration.CHECKPOINT_PATH, "best_online_crnn_model.pth"); torch.save(model.state_dict(), best_model_save_path)
                      else:
                          patience_counter += 1; logger.info(f"Keine Verbesserung bei '{es_metric_key}'. Geduld: {patience_counter}/{crnn_configuration.EARLY_STOPPING_PATIENCE} (Best: {best_val_metric_value:.4f} @ Ep {best_epoch})")
                          if patience_counter >= crnn_configuration.EARLY_STOPPING_PATIENCE: logger.warning("EARLY STOPPING!"); early_stop_triggered = True
            epoch_duration = time.time() - epoch_start_time; logger.info(f"Epoche {epoch} Dauer: {epoch_duration:.2f} Sek.")
            save_interval = crnn_configuration.SAVE_CHECKPOINT_INTERVAL
            if save_interval > 0 and (epoch % save_interval == 0 or epoch == crnn_configuration.EPOCHS or early_stop_triggered): logger.info(f"Speichere History/Plots (Epoche {epoch})..."); save_training_history(training_history); plot_training_curves(training_history)
        ...
        else: logger.info(f"Grund: Maximale Epochenzahl ({crnn_configuration.EPOCHS}) erreicht.")
