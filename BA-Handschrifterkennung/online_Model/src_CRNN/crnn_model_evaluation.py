...
try:
    ...
    from crnn_model_training import OnlineHandwritingDataset, online_collate_fn
except ImportError as e:
    logging.basicConfig(level=logging.ERROR)
    logger_fallback = logging.getLogger(__name__)
    logger_fallback.error(f"Kritischer Fehler beim relativen Import in crnn_model_evaluation: {e}. Paketstruktur/ PYTHONPATH prüfen!", exc_info=True)
    sys.exit(1)
...
def save_sample_predictions_text(relative_sample_paths, true_labels, pred_labels, split_name, num_samples=30):
    ...
    online_utils.create_directory(crnn_configuration.RESULTS_PATH) # Util Funktion korrekt aufrufen
    txt_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{split_name}_sample_predictions_final.txt")
...
def plot_error_distribution(all_true_strs, all_pred_strs, split_name):
    ...
    try:
        ...
        hist_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{split_name}_error_distribution_hist.png"); plt.tight_layout(); plt.savefig(hist_path); plt.close(fig)
        ...
        boxplot_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{split_name}_error_boxplot.png"); plt.tight_layout(); plt.savefig(boxplot_path); plt.close(fig)
...
def plot_length_vs_metric(all_true_strs, all_pred_strs, metric_name, split_name):
    ...
    try:
        ...
        plot_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{split_name}_length_vs_{metric_name.lower()}.png"); plt.savefig(plot_path); plt.close()
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            try:
                ...
                for i in range(batch_size):
                    ...
                    true_str = online_utils.decode_label_list(label_indices, crnn_configuration.IDX_TO_CHAR)
...
def evaluate_online_model(model_path_to_evaluate=None):
    ...
    try:
        device = torch.device(crnn_configuration.DEVICE); logger.info(f"Starte Online-Evaluation auf: {device}")
        ...
        else: model_path = os.path.join(crnn_configuration.CHECKPOINT_PATH, "best_online_crnn_model.pth")
        ...
        criterion = nn.CTCLoss(blank=crnn_configuration.BLANK_IDX, reduction='mean', zero_infinity=True)
        ...
        df_test = online_utils.load_manifest(crnn_configuration.CURRENT_TEST_MANIFEST)
        ...
        eval_batch_size = max(1, crnn_configuration.BATCH_SIZE); logger.info(f"Verwende Eval-Batchsize: {eval_batch_size}")
        train_loader = DataLoader(train_dataset, batch_size=eval_batch_size, shuffle=False, collate_fn=online_collate_fn, num_workers=crnn_configuration.NUM_WORKERS) if train_dataset else None
        val_loader = DataLoader(val_dataset, batch_size=eval_batch_size, shuffle=False, collate_fn=online_collate_fn, num_workers=crnn_configuration.NUM_WORKERS) if val_dataset else None
        test_loader = DataLoader(test_dataset, batch_size=eval_batch_size, shuffle=False, collate_fn=online_collate_fn, num_workers=crnn_configuration.NUM_WORKERS) if test_dataset else None
        ...
        all_metrics_path = os.path.join(crnn_configuration.METRICS_PATH, "all_split_metrics_final.json")
        try:
        ...
        for split_name, predictions_df in all_split_predictions.items():
             if not predictions_df.empty and 'error' not in predictions_df.columns:
                samples_csv_path = os.path.join(crnn_configuration.RESULTS_PATH, f"{split_name}_all_predictions_final.csv")
        ...
        report = { # (Report-Inhalt unverändert) ...
            ...
            'dataset_info': { 'train_size': len(df_train), 'val_size': len(df_val), 'test_size': len(df_test),
                              'feature_dim': crnn_configuration.FEATURE_DIM, 'max_seq_len': crnn_configuration.MAX_SEQ_LEN,
                              'char_list_size': len(crnn_configuration.CHAR_LIST), 'num_classes_incl_blank': crnn_configuration.NUM_CLASSES },
            'evaluation_metrics': serializable_metrics_all,
            'evaluation_timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        report_path = os.path.join(crnn_configuration.RESULTS_PATH, "evaluation_report_final.json")
        try: 
