...
def load_feature_pseudo_image(absolute_bin_path):
    ...
    # (Code unverändert)
    target_h = crnn_configuration.CNN_INPUT_HEIGHT; target_w = crnn_configuration.CNN_INPUT_WIDTH
    ...
    try:
        ...
        expected_bytes_multiple = 4 * crnn_configuration.FEATURE_DIM
        if len(raw_bytes) % expected_bytes_multiple != 0: logger.error(f"Datei {os.path.basename(absolute_bin_path)}: Größe ({len(raw_bytes)}) != multiple of (4 * {crnn_configuration.FEATURE_DIM}). Corrupted?"); return None
        ...
        num_timesteps = features_flat.numel() // crnn_configuration.FEATURE_DIM
        if num_timesteps == 0: logger.warning(f"0 Zeitstempel nach Reshape: {absolute_bin_path}"); return None
        features = features_flat.reshape(num_timesteps, crnn_configuration.FEATURE_DIM) # [T, F]
        if num_timesteps > 1:
...
def encode_label(label_str, char_to_idx):
    ...
    for char in label_str:
        ...
        if idx is not None and idx != crnn_configuration.BLANK_IDX: encoded.append(idx)
        elif char not in unknown_chars and idx != crnn_configuration.BLANK_IDX :
...
def decode_label_list(encoded_label_list, idx_to_char):
    ...
    # (Code unverändert)
    return "".join([idx_to_char[idx] for idx in encoded_label_list if idx != crnn_configuration.BLANK_IDX and idx in idx_to_char])

def decode_ctc_output(predictions_tensor, idx_to_char):
    """Dekodiert CTC Output (Best Path)."""
    # (Code unverändert)
    blank_idx = crnn_configuration.BLANK_IDX
...
def save_metrics_to_file(metrics_dict, split_name, output_dir=None, filename_suffix=""):
    if output_dir is None: output_dir = crnn_configuration.METRICS_PATH
...
def plot_metrics_comparison(all_split_metrics):
    ...
    plt.tight_layout(pad=2.0); plot_path = os.path.join(crnn_configuration.RESULTS_PATH, "metrics_comparison.png")
