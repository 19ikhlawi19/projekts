...
try:
    from stroke_preprocessing import preprocess_handwriting
    from stroke_feature_extraction import calculate_feature_vector_sequence, _get_expected_num_features
    from handwriting_utilities import create_directory, clean_relative_path
...
def create_initial_dataframe(data_folder_abs):
    ...
    for txt_file in tqdm(txt_files, desc="Parsing TXT files"):
        ...
        for i, line in enumerate(transcript_lines):
            ...
            # Pfad relativ zu BASE_PATH, basierend auf crnn_configuration.DATA_FOLDER_NAME
            xml_path_rel_to_base = os.path.join(crnn_configuration.DATA_FOLDER_NAME, 'lineStrokes', xml_dir_rel, xml_filename).replace("\\", "/")
...
def process_sample(args_tuple):
    ...
    bin_path_rel = xml_path_rel.replace(
        f"{crnn_configuration.DATA_FOLDER_NAME}/lineStrokes", f"{crnn_configuration.DATA_FOLDER_NAME}/{bin_folder_rel}"
...
def process_and_save_features(df, dataset_name, bin_folder_rel, norm_args, feat_args, use_parallel=True):
    ...
    bin_output_dir_abs = os.path.join(base_path, crnn_configuration.DATA_FOLDER_NAME, bin_folder_rel)
    # Verwende die direkt importierte Funktion
...
def run_data_preparation():
    ...
    DATA_FOLDER_NAME_LOCAL = os.path.basename(crnn_configuration.DATA_FOLDER) # 'data'
    BIN_FOLDER_REL = crnn_configuration.BIN_FEATURE_FOLDER_NAME # 'bin_features'
    OUTPUT_EXCEL_PREFIX = crnn_configuration.DATA_PREFIX # 'iam_prepared'
    ...
    if calculated_dim != crnn_configuration.FEATURE_DIM:
        logger.error(f"FATAL: Calculated feature dim ({calculated_dim} from FEAT_ARGS) != crnn_configuration.FEATURE_DIM ({crnn_configuration.FEATURE_DIM}). Update config or FEAT_ARGS.")
        sys.exit(1)
    logger.info(f"Using feature dimension: {crnn_configuration.FEATURE_DIM}")
    logger.info(f"Using preprocessing steps: {NORM_ARGS}")
    logger.info(f"Using feature extraction steps: {FEAT_ARGS}")

    SPLIT_RANDOM_STATE = crnn_configuration.RANDOM_SEED
    TEST_SIZE = crnn_configuration.TEST_SPLIT
    VAL_SIZE = crnn_configuration.VAL_SPLIT
    USE_PARALLEL = (crnn_configuration.NUM_WORKERS > 1)

    # --- Setup ---
    base_path = crnn_configuration.BASE_PATH
    data_folder_abs = crnn_configuration.DATA_FOLDER # Verwende absoluten Pfad aus config
    output_bin_folder_abs = crnn_configuration.BIN_FEATURE_PATH # Verwende absoluten Pfad aus config
    ...
    train_excel_path = crnn_configuration.TRAIN_MANIFEST_FILE
    val_excel_path = crnn_configuration.VAL_MANIFEST_FILE
    test_excel_path = crnn_configuration.TEST_MANIFEST_FILE
    try:
