...
try:
    ...
    import handwriting_utilities as online_utils
    logging.warning("Relativer Import für handwriting_utilities fehlgeschlagen. Versuche direkten Import.")
...
class CRNN(nn.Module):
    ...
    """
    def __init__(self, num_classes=crnn_configuration.NUM_CLASSES, rnn_hidden_size=256, rnn_dropout=0.3):
        super().__init__()
        self.num_classes = num_classes
        self.rnn_hidden_size = rnn_hidden_size
        self.rnn_dropout = rnn_dropout
        self.cnn_input_channels = crnn_configuration.CNN_INPUT_CHANNELS
        self.cnn_input_height = crnn_configuration.CNN_INPUT_HEIGHT
        self.cnn_input_width = crnn_configuration.CNN_INPUT_WIDTH
...
def build_online_crnn_model():
    ...
    try:
        model = CRNN(
            num_classes=crnn_configuration.NUM_CLASSES,
            rnn_hidden_size=256,
            rnn_dropout=0.3
        )
        ...
        logger.info(f"  Klassen (inkl. Blank): {crnn_configuration.NUM_CLASSES}, RNN Hidden: {model.rnn_hidden_size}, SeqLen (CNN out): {model.sequence_length}")
        try:
            ...
            online_utils.create_directory(crnn_configuration.CHECKPOINT_PATH)
            arch_path = os.path.join(crnn_configuration.CHECKPOINT_PATH, "model_architecture_online.txt")
